...
bool setup_prepared_stmt(MYSQL_STMT** stmt, char* statement, MYSQL* conn)
{
	...
	*stmt = mysql_stmt_init(conn);
	if (*stmt == NULL)
	{
		print_error(conn, (char*)"Could not initialize statement handler");
		return false;
	}

	if (mysql_stmt_prepare(*stmt, statement, strlen(statement)) != 0) {
		print_stmt_error(*stmt, (char*)"Could not prepare statement");
		return false;
	}
...
static void dump_result_set_header(MYSQL_RES* res_set)
{
	...
	mysql_field_seek(res_set, 0);	/* it's like seek() function for file descriptors */

	for (i = 0; i < mysql_num_fields(res_set); i++) {
		field = mysql_fetch_field(res_set);		/* type of each field (column) */
		col_len = strlen(field->name);			/* length of the name of each field */
...
void dump_result_set(MYSQL* conn, MYSQL_STMT* stmt, char* title)
{
	...
	MYSQL_RES* rs_metadata;		/* to get metadata of result set */
	MYSQL_TIME* date;
	size_t attr_size;
	...
	if (num_fields > 0) {
		...
		if ((rs_metadata = mysql_stmt_result_metadata(stmt)) == NULL) {
			finish_with_stmt_error(conn, stmt, (char*)"Unable to retrieve result metadata\n", true);
		}

		dump_result_set_header(rs_metadata);		/* function that allows to print pretty tables on stdout */

		fields = mysql_fetch_fields(rs_metadata);

		rs_bind = (MYSQL_BIND*)malloc(sizeof(MYSQL_BIND) * num_fields);
		if (!rs_bind) {
			finish_with_stmt_error(conn, stmt, (char*)"Cannot allocate output buffers\n", true);
		}
		...
		if (mysql_stmt_bind_result(stmt, rs_bind)) {
			finish_with_stmt_error(conn, stmt, (char*)"Unable to bind output parameters\n", true);
		}
...
void bind_par(MYSQL_BIND param[], struct param_type* paramStructPtr) {
	struct param_type paramStruct = *paramStructPtr;

	// Prepare parameters
	memset(param, 0, sizeof(param));

	int count;		// Number of entries of param[] that have been initalized

	for (int i = 0; i < paramStruct.numVarchar; i++) {
		param[i].buffer_type = MYSQL_TYPE_VAR_STRING;
		param[i].buffer = paramStruct.varchar[i];
		param[i].buffer_length = strlen(paramStruct.varchar[i]);
	}
	count = paramStruct.numVarchar;

	for (int i = 0; i < paramStruct.numDate; i++) {
		param[i + count].buffer_type = MYSQL_TYPE_DATE;
		param[i + count].buffer = (char *)paramStruct.date[i];		// Pointer to a MYSQL_TIME data structure
		param[i + count].buffer_length = sizeof(MYSQL_TIME);
	}
	count = count + paramStruct.numDate;

	for (int i = 0; i < paramStruct.numTime; i++) {
		param[i + count].buffer_type = MYSQL_TYPE_TIME;
		param[i + count].buffer = (char *)paramStruct.time[i];
		param[i + count].buffer_length = sizeof(MYSQL_TIME);
	}
	count = count + paramStruct.numTime;

	for (int i = 0; i < paramStruct.numInt; i++) {
		param[i + count].buffer_type = MYSQL_TYPE_LONG;
		param[i + count].buffer = paramStruct.integer[i];
		param[i + count].buffer_length = sizeof(int);
	}
	count = count + paramStruct.numInt;

	for (int i = 0; i < paramStruct.numFloat; i++) {
		param[i + count].buffer_type = MYSQL_TYPE_FLOAT;
		param[i + count].buffer = paramStruct.f;
		param[i + count].buffer_length = sizeof(float);
	}
	count = count + paramStruct.numFloat;

	for (int i = 0; i < paramStruct.numBool; i++) {
		param[i + count].buffer_type = MYSQL_TYPE_SHORT;
		param[i + count].buffer = paramStruct.b;
		param[i + count].buffer_length = sizeof(short int);
	}
}
